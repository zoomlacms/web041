                ...
                case "mb_getdata"://按条件筛选内容并返回
                    {
                        ...
                        string fields = "A.GeneralID AS id,A.title,A.hits,A.CreateTime AS cdate,B.wxsize,B.wxico,B.wxbk,B.flag";//固定返回这些字段
                        ...
                        DataTable dt = DBCenter.SelPage(new PageSetting()
                        {
                           ...
                           //DBCenter.JoinQuery(fields, "ZL_CommonModel", "ZL_C_Article", "A.ItemID=B.ID", where, "A.GeneralID DESC", sp.ToArray());
                        retMod.result = JsonConvert.SerializeObject(dt);
                        retMod.retcode = M_APIResult.Success;
                    }
                    break;

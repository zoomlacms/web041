...
function help_show(helpid) {
    ...
    var str = "<div id='help_content'  style='z-index:999;'></div><div id='help_hide'  style='z-index:999;'><a onclick='help_hide(Num)' style='width:20px;color:#666' title='关闭'><span class='glyphicon glyphicon-remove'></span></a></div> "; newDiv.innerHTML = str; newDiv.setAttribute("Id", "help_div" + Num); nn = Num - 1
...
user.sel = function (select, source) {
    var url = "";
    switch (source)
    {
        case "plat":
            url = "/Plat/Common/SelGroup.aspx#" + select;
            break;
        case "OA":
            url = "/Common/Dialog/SelGroup.aspx?source=oa#" + select;
            break;
        default:
            url = "/Common/Dialog/SelGroup.aspx#" + select;
            break;
    }
    comdiag.maxbtn = false;
